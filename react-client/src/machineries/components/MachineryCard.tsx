...
export default function MachineryCard(props: Machinery) {
    ...
    const {type, name, id, status, location} = props;
            ...
            <HStack alignItems="flex-start" justifyContent="left" flexDirection="row" minWidth="100%">
                <Box>
                    <Avatar
                        size="lg"
                        src={
                            require(`/src/assets/machineries/${type}.png`)
                        }
                        mb={4}
                        pos="relative"
                    >
                        <Tooltip label={status === 'active' ? 'Active' : 'Inactive'}>
                            <Box
                                w="5"
                                h="5"
