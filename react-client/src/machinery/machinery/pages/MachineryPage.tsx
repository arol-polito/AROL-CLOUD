...
import type Sensor from '../../dashboard/models/Sensor'
...
export default function MachineryPage (props: DashboardProps) {
  ...
  useEffect(() => {
    if ((machinery == null) || !machineryUID) return

    async function getData () {
      try {
        const result = await machineryService.getMachinerySensors(machineryUID || '')
        ...
        <Box w="full">
            <Breadcrumb mb={2}>
                ...
                <BreadcrumbItem isCurrentPage={props.type === 'landing'}>
                    <BreadcrumbLink
                        onClick={(e) => {
                          breadcrumbNavigate(e, `/machinery/${machineryUID}`, props.type === 'landing')
                        }}>
                        {machineryUID}
                    </BreadcrumbLink>
                </BreadcrumbItem>
