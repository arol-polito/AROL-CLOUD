...
export default function SensorsModal(props: SensorsModalProps) {
    ...
    function handleMonitorSensorsClicked() {
        ...
        props.setSensorsMonitoring((val) => {
            ...
            let aggregations = sensorAggregations.filter((aggregation) => (aggregation !== "none"))
            ...
            for (const [key, value] of Object.entries(selectedSensors)) {
                value.forEach((entry) => {
                    ...
                    let foundEntry = val.sensors[key].find((el) => (el.headNumber === headNum))
                    if (foundEntry) {
                        entry.sensorNames.forEach((sensorName) => {

                            let foundSensorEntry = foundEntry!!.sensorNames.find((el) => (el.name === sensorName.name))
                            ...
                            sensorAggregations.map((value, index) => (
                                ...
                                >
                                    <HStack w={"full"}>
                                        ...
                                        <Select value={value}
                                                onChange={(e) => (handleAggregationSelected(e.target.value, index))}>
...
function SensorEntry(props: SensorEntryProps) {
    ...
    useEffect(() => {
        ...
        for (const entry of props.sensorsMonitoring) {
            if (entry.sensorNames.find((el) => (el.name === props.sensor.internalName))) {
                numHeadsSelected++
            }
        }
    ...
    function handleSelectAllSensorsButtonClick() {
        ...
        props.setSelectedSensors((val) => {
            ...
            Array.from(Array(numHeads)).forEach((_, indexHead) => {
                ...
                let foundEntry = val[sensorCategory].find((el) => (el.headNumber === currHeadNum))
                if (foundEntry) {
...
function SensorDescription(props: { sensor: Sensor, setDescriptionExpanded: React.Dispatch<React.SetStateAction<boolean>> }) {
    return <HStack
        w={"full"}
        alignItems={"top"}
        justifyContent={"space-between"}
...
function HeadMechEntry(props: HeadMechEntryProps) {
    ...
    useEffect(() => {
        let foundEntry = props.selectedSensors.find((el) => (el.headNumber === props.indexHead))
        if (foundEntry) {
            if (foundEntry.sensorNames.find((el) => (el.name === props.sensor.internalName)) !== undefined) {
    ...
    useEffect(() => {
        let foundEntry = props.sensorsMonitoring.find((el) => (el.headNumber === props.indexHead))
        if (foundEntry) {
            if (foundEntry.sensorNames.find((el) => (el.name === props.sensor.internalName)) !== undefined) {
    ...
    function handleSelectSensorButtonClick() {
        ...
        props.setSelectedSensors((val) => {
            let foundEntry = val[sensorCategory].find((el) => (el.headNumber === props.indexHead))
            if (foundEntry) {
                foundEntry.sensorNames.push({
                    name: props.sensor.internalName,
